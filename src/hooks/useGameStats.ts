...
import * as GameStatistics from '../utils/GameStatistics';
...
const calculateStats = createSelector(
  ...
  (gameData: PlayerGameData, currentLeg: PlayerGameData['throwHistory'], currentSet: PlayerGameData['throwHistory']): PlayerStats => {
    if (!gameData?.throwHistory) {
      return {
        averages: { overall: 0, leg: 0, set: 0, firstNine: 0 },
        checkouts: { percentage: 0, successful: 0, attempts: 0 },
        legsWon: {
          nine: 0, tenToTwelve: 0, thirteenToFifteen: 0,
          sixteenToTwenty: 0, twentyOneToTwentyFive: 0,
          twentySixToThirty: 0, thirtyPlus: 0
        },
        scoring: {
          oneEighty: 0, oneFortyPlus: 0, oneHundredPlus: 0,
          sixtyPlus: 0, fortyPlus: 0, twentyPlus: 0, oneToTwenty: 0
        }
      };
    }
...
export const useGameStats = (gameData: PlayerGameData): PlayerStats => {
  const { cacheStats, getCachedStats } = useStatsCache();
  
  return useMemo(() => {
    // Prøv at hente fra cache først
    if (gameData?.playerId) {
      const cachedStats = getCachedStats(gameData.playerId);
      if (cachedStats) {
        return cachedStats;
      }
    }
